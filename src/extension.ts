...
let bsConfigPath: string | undefined;
...
async function startLiveServer(targetResource?: vscode.Uri, rootOverride?: string) {
    ...
    if (useXamppApache) {
        ...
        const port = 3000;
        // Create temporary browser-sync config with no-cache middleware
        try {
            bsConfigPath = createBrowserSyncConfig(workspacePath, port, hostBind);
        } catch (err) {
            console.error('Failed to create browser-sync config:', err);
            bsConfigPath = undefined;
        }

        let bsCommand = bsConfigPath
            ? `npx browser-sync start --config "${bsConfigPath}" --host ${hostBind} --port ${port} --no-open`
            : `npx browser-sync start --server "${workspacePath}" --host ${hostBind} --port ${port} --no-open --files "${workspacePath}"`;
        if (httpsConfigPath) {
    ...
    const serverType = useXamppApache ? 'XAMPP Apache Server' : (useXampp && isPhpProject ? 'XAMPP PHP Server' : (isPhpProject ? 'PHP Server' : 'Live Server'));
    // Use a transient status bar message instead of modal info
    vscode.window.setStatusBarMessage(`Starting ${serverType}...`, 4000);
    ...
    if (liveServerProcess) {
        if (isPhpProject) {
            ...
            setTimeout(() => {
                ...
                vscode.env.openExternal(vscode.Uri.parse(serverUrl));
                vscode.window.setStatusBarMessage(`${serverType} started at ${serverUrl}`, 6000);
            }, 1500);
        } else {
...
function stopLiveServer(options: { silent?: boolean } = {}) {
    ...
    if (useXamppApache && !liveServerProcess && !silent) {
        vscode.window.setStatusBarMessage('Apache continues running in XAMPP Control Panel. Please stop it manually if needed.', 6000);
    }
    ...
    if (!silent) {
        vscode.window.setStatusBarMessage(`${serverType} stopped`, 4000);
    }

    // Cleanup temporary browser-sync config if created
    try {
        if (bsConfigPath && fs.existsSync(bsConfigPath)) {
            fs.unlinkSync(bsConfigPath);
            bsConfigPath = undefined;
        }
    } catch (err) {
        console.error('Failed to cleanup bsConfigPath:', err);
    }
}

function createBrowserSyncConfig(workspacePath: string, port: number, hostBind: string): string {
    // Write a temporary JS config for browser-sync with middleware that adds no-cache headers
    const tmp = os.tmpdir();
    const fileName = `live-server-bs-config-${Date.now()}.js`;
    const filePath = path.join(tmp, fileName);

    const jsFile = `module.exports = {\n    server: {\n        baseDir: ${JSON.stringify(workspacePath)},\n        middleware: [function (req, res, next) {\n            res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');\n            res.setHeader('Pragma', 'no-cache');\n            res.setHeader('Expires', '0');\n            res.setHeader('Surrogate-Control', 'no-store');\n            next();\n        }]\n    },\n    files: [${JSON.stringify(workspacePath)}],\n    notify: false,\n    ui: false,\n    port: ${port},\n    host: ${JSON.stringify(hostBind)}\n};\n`;

    fs.writeFileSync(filePath, jsFile, { encoding: 'utf8' });
    return filePath;
}
